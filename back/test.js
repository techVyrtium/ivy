...
async function executeWorkflow(items) {
  ...
  for (const item of items) {
    try {
      ...
      console.log("Processing AI Agent Output:", JSON.stringify(aiAgentOutput, null, 2));
      
      // Extract the keywords for searching
      let keywords = [];
      
      if (aiAgentOutput.combined_key_words && Array.isArray(aiAgentOutput.combined_key_words)) {
        keywords = aiAgentOutput.combined_key_words;
      } else if (aiAgentOutput.english_key_words && Array.isArray(aiAgentOutput.english_key_words)) {
        keywords = aiAgentOutput.english_key_words;
      } else if (aiAgentOutput.spanish_key_words && Array.isArray(aiAgentOutput.spanish_key_words)) {
      ...
      if (!keywords || keywords.length === 0) {
        ...
        if($input.first().json.output.context.domain){
          keywords.push($input.first().json.output.context.domain)
        }
        else{
        returnItems.push({
          json: {
      ...
      if (budgetMin !== null && budgetMax !== null) {
        exactMatchQuery.$and.push({ usd: { $gte: budgetMin, $lte: budgetMax } });
      ...
      if (budgetMin !== null && budgetMax !== null) {
        alternativeMatchQuery.$and.push({ usd: { $gte: budgetMin, $lte: budgetMax } });
      } else if (budgetMax !== null) {
      ...
      const similarProductQuery = {
        $and: [
          ...
          { $or: [
              ...keywords.map(keyword => ({ category: { $regex: keyword, $options: 'i' } })),
              ...keywords.map(keyword => ({ service: { $regex: keyword, $options: 'i' } }))
            ]
          }
        ]
      };
      ...
      if (budgetMin !== null && budgetMax !== null) {
        similarProductQuery.$and.push({ usd: { $gte: budgetMin, $lte: budgetMax } });
      } else if (budgetMax !== null) {
      ...
      returnItems.push({
        json: {
          ...
          product_or_service_id: $input.first().json.output.product_or_service_id,
